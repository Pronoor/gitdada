...
note 'Admin page';
{
  ...
  # Login fail
  $t->post_ok('/?op=login', form => {id => 'admin', password => 'b'});
  $t->content_like(qr/User name or password is wrong/);

  # Login success
  $t->post_ok('/?op=login', form => {id => 'admin', password => 'a'});
...
note 'Reset password';
{
  ...
  # Login as kimoto
  $t->post_ok('/?op=login', form => {id => 'kimoto1', password => 'a'});
  ...
  # Login as kimoto
  $t->get_ok('/_logout');
  $t->post_ok('/?op=login', form => {id => 'kimoto1', password => 'b'});
  $t->get_ok('/')->content_like(qr/kimoto1/);
}
...
note 'Private repository and collaborator';
{
  ...
  # Login as kimoto
  $t->post_ok('/?op=login', form => {id => 'kimoto', password => 'a'});
  ...
  # Login as kimoto2
  $t->post_ok('/?op=login', form => {id => 'kimoto2', password => 'a'});
